...
- **提交范围**：`ff03d0b`，3 个文件全是本票的；提交后工作区干净。全量套件 **223 项全绿**。
- **一处必须写进历史的连带影响（本票的正面后果，同时短暂放大一个既有缺口）**：旧代码的 `self` 事件走 `boot()`，而 `boot()` 会**关掉旧流并重建**（`git show ff03d0b^:public/app.js` 第 123–126 行，注释写着「自己的角色/账号被改动：重新走一次 bootstrap，权限变化立刻生效」）。也就是说：角色变更后客户端带着**新角色**重新建连，服务端「连接时的 role 快照」问题一直被这个副作用**掩盖**着。07 移除这条重建正是它该做的，于是那个服务端缺口**从被掩盖变成暴露**，直到工单 04（viewer 权威）落地。
- **影响面经核实是元数据级、不是数据级**：`broadcastAdmin` 只发 `{ scope:'admin', kind }`、不带任何数据（`grep` 确认唯一调用点在 `sse.js` 的 `publish`），而客户端收到 admin 事件后的动作是走 REST 重新拉取，那条路**按当前角色过滤**。所以被降权者能推断出「某个 admin 范围的东西变了」，但拿不到任何越权数据。**修法是 04，已在段一队列里；在此之前不要把实例长期开着给真实用户用。**
- 本票自己记的那条服务端缺口（删账号级联删掉指向某人的邀请时，服务端只给 admin 发事件，非 admin 的角标要等下一次全量）归属服务端：会在 04 / 05 收口时核对是否被覆盖；若没有，另开票。
