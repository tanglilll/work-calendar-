...
这里记的是**代码和测试照不出来、只在这台机器上成立**的事实：宿主的进程管理、本机网络、浏览器侧的 cookie 归属、内置浏览器支持哪些原生对话框、`node:sqlite` 的返回值细节。目的很直白——下一个人不用去 `%TEMP%` 或对话记录里考古，也不必把同一个坑再踩一遍。
...
## 6. IAB 里原生 `confirm` 可用、`prompt()` 被禁用：二次确认不要装页面级替身

**症状（现象）**

- `prompt()` 在内置浏览器（IAB）里**被禁用**：点依赖它的按钮（实测：管理面板的「转移事项」）只弹一条红色错误提示 **`prompt() is not supported.`**，面板不变、流程走不下去——这条界面路径在这个环境里等于断的。凡还有 `prompt()` 调用点的界面，先假定它在这里不可用。
- 原生 `confirm` **可用**：浏览器自带的对话框接口能读到 `type: confirm` 并接受，页面照常按返回值分支。实测接口是 `getJsDialog()` → `confirm.accept()`；配合 DOM 派发的点击，删除、归档、删账号这些带二次确认的路径都走通过。**通过二次确认不必给页面装 `window.confirm` 替身。**

**为什么重要**

- 装 `window.confirm` 替身等于**篡改页面行为**：确认框不再出现，返回值由替身给定。上一轮装上 spy 后自己记下了这句话——「它确实改变了行为：替用户点了确认」（closeout 工单 04 的「标记完成」路径）；替身失效或返回值不对时，就是点击被静默吞掉、界面毫无反应。两种情形里，使用者以为自己在观察页面行为，实际看到的是替身的行为，**据此得出的结论不可信**。
- 而且它**恢复不干净**：`confirm` 是 `window` 自己的属性（不在原型链上），`delete window.confirm` 删掉的只是替身，原生实现不会回来——之后这个页面上的二次确认永久失效，看起来像功能坏了。
- 换来的必要能力是零：原生 `confirm` 本来就能处理（能读 type、能 accept），替身并不解锁任何「不装就做不到」的验证。

**正确做法**

- 二次确认一律用**浏览器自带的对话框接口**（`getJsDialog()` → 读 `type`、`accept()` / `dismiss()`）放行，不碰页面的 `window.confirm`。一处如实记下的代价：该接口只暴露 accept / dismiss，**读不到 message**，所以「确认框的视觉呈现与文案」不在观测范围内（与 c4 同口径）。
- 断言确实需要页内探针（文案、请求体、toast）时，**新开一个标签页**、在那边装探针并断言，验完 `close()`；不要把探针留在被观察的页面上。新标签页共享同一 cookie 桶、打开即处于登录态（机制见 §5），但页面是全新加载、不带探针。
- 驱动细节（哪种点击方式可靠、实例与标签页如何隔离）以 `.scratch/browser-verification/spec.md` 为唯一出处，本条不转载。

**来源**（2026-09-18，上一轮实测记录；本次为纯文档整理，未重跑浏览器）

- `prompt()` 被禁用：verification-fixes 工单 03（`.scratch/verification-fixes/issues/03-transfer-dialog-view.md`）在内置浏览器实测——点「转移事项」只弹 `prompt() is not supported.`，流程走不下去，该路径因此改成对话框内的候选列表。
- 原生 `confirm` 可用：verification-fixes 工单 02 的「本轮新确认的环境事实」与工单 03 的「路径 0（转移前删除被拒）」都记了它（`getJsDialog()` → `confirm.accept()` 放行，**没有**装 `window.confirm` 替身）；arch-deepening-2 的段界冒烟里，工单 05 的删除路径（「删除已归档事项」允许，见 `.scratch/arch-deepening-2/spec.md` 的「段界冒烟结果」服务端段）同样凭它走通。
- 替身的代价：closeout 工单 04（`.scratch/closeout/issues/04-version-conflict-ui.md`）记下 spy「确实改变了行为：替用户点了确认」；browser-verification 的 c4（`.scratch/browser-verification/issues/03-c4-archive-confirm.md`）也用了同样的 spy，并记下「原生确认框的视觉呈现未被观测」。`delete window.confirm` 恢复不了原生实现来自上一轮的现场教训（本轮未复现）。
